...
from mmdet.det_core import bbox_overlaps
...
def isr_p(cls_score,
    ...
    gts = list()
    last_max_gt = 0
    for i in range(len(sampling_results)):  # batch长度
        gt_i = sampling_results[i].pos_assigned_gt_inds
        gts.append(gt_i + last_max_gt)  # 因为batch维度也拉伸了，故需要加上last_max_gt，后面才对的上
        if len(gt_i) != 0:
            last_max_gt = gt_i.max() + 1
    gts = torch.cat(gts)  # 正样本对应的gt bbox在整个gt bbox中的索引
    ...
    if rois.size(-1) == 5:
        ...
        pos_rois = rois[pos_label_inds]  # 正样本对应的anchor

    if bbox_pred.size(-1) > 4:
        bbox_pred = bbox_pred.view(bbox_pred.size(0), -1, 4)
        pos_delta_pred = bbox_pred[pos_label_inds, pos_labels].view(-1, 4)
    else:
        pos_delta_pred = bbox_pred[pos_label_inds].view(-1, 4)  # 正样本对应的预测值
    ...
    # pos_labels=(M,)，存储的是0-cls_num的类别编号，bincount统计每个类别出现的频数(返回长度改变了)
    # 例如input=tensor([4, 3, 6, 3, 4]),torch.bincount(input),out=tensor([0, 0, 0, 2, 2, 0, 1])
    max_l_num = pos_labels.bincount().max()  # 计算类别出现最多的个数
    for label in pos_labels.unique():  # 遍历不同类别
        l_inds = (pos_labels == label).nonzero().view(-1)
        l_gts = gts[l_inds]  # 对应类别的所有gt bbox的索引位置
        for t in l_gts.unique():  # 遍历每个gt bbox，相当于分组排序
            t_inds = l_inds[l_gts == t]  # 某个gt bbox的所有正样本anchor负责位置索引
            t_ious = ious[t_inds]  # 对应的iou值，然后进行组内排序
            # 假设t_ious=[0.6,0.7,0.1,0.8,0.4],那么t_iou_rank_idx=[3,1,0,4,2]
            _, t_iou_rank_idx = t_ious.sort(descending=True)
            # t_iou_rank=[2,1,4,0,3]
            _, t_iou_rank = t_iou_rank_idx.sort()  # t_iou_rank保存序号，方便后面计算
            # 排名越前，值越低表示权重越大，并且不同gt bbox的ious[t_inds]其top1...topk值是一样的
            # 后面在组间排序就非常简单了，一次排序就可以实现a_top1,b_top1,a_top2,b_top2,.....
            ious[t_inds] += max_l_num - t_iou_rank.float()
        # 组间排序,就是一次排序就行，因为ious里面的每个值都已经被赋予新的取值了
        # a_top1,b_top1,a_top2,b_top2,.....
        l_ious = ious[l_inds]
        _, l_iou_rank_idx = l_ious.sort(descending=True)
        _, l_iou_rank = l_iou_rank_idx.sort()  # IoU-HLR
        # linearly map HLR to label weights 线性映射
    ...
    label_weights[pos_label_inds] = pos_imp_weights  # 最终就是把每个位置的权重赋予新值即可
...
def carl_loss(cls_score,
    ...
    carl_loss_weights *= weight_ratio  # 得到新的权重
