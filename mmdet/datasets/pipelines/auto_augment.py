...
class Rotate(object):
    ...
    Args:
        ...
        center (int | float | tuple[float]): Center point (w, h) of the
            ...
            image will be used. Same in ``cv_core.imrotate``.
    ...
    def _rotate_img(self, results, angle, center=None, scale=1.0):
        ...
        Args:
            ...
            angle (float): Rotation angle in degrees, positive values
                mean clockwise rotation. Same in ``cv_core.imrotate``.
            center (tuple[float], optional): Center point (w, h) of the
                rotation. Same in ``cv_core.imrotate``.
            scale (int | float): Isotropic scale factor. Same in
                ``cv_core.imrotate``.
        """
        for key in results.get('img_fields', ['img']):
            img = results[key].copy()
            img_rotated = cv_core.imrotate(
...
class Translate(object):
    ...
    def _translate_img(self, results, offset, direction='horizontal'):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key].copy()
            results[key] = cv_core.imtranslate(
    ...
    def _translate_seg(self,
        ...
        for key in results.get('seg_fields', []):
            seg = results[key].copy()
            results[key] = cv_core.imtranslate(seg, offset, direction,
...
class ColorTransform(object):
    ...
    def _adjust_color_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            img = results[key]
            results[key] = cv_core.adjust_color(img, factor).astype(img.dtype)
...
class EqualizeTransform(object):
    ...
    def _imequalize(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = cv_core.imequalize(img).astype(img.dtype)
...
class BrightnessTransform(object):
    ...
    def _adjust_brightness_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = cv_core.adjust_brightness(img,
...
class ContrastTransform(object):
    ...
    def _adjust_contrast_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = cv_core.adjust_contrast(img, factor).astype(img.dtype)
