...
class Resize(object):
    ...
    def random_select(img_scales):
        ...
        assert mmdet.cv_core.is_list_of(img_scales, tuple)
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                ...
                # a real fix should be done in the mmdet.cv_core.imrescale in the future
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = mmdet.cv_core.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = mmdet.cv_core.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class RandomFlip(object):
    ...
    def __init__(self, flip_ratio=None, direction='horizontal'):
        if isinstance(flip_ratio, list):
            assert mmdet.cv_core.is_list_of(flip_ratio, float)
        ...
        if isinstance(direction, str):
            ...
            assert mmdet.cv_core.is_list_of(direction, str)
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            # flip image
            for key in results.get('img_fields', ['img']):
                results[key] = mmdet.cv_core.imflip(
...
class Pad(object):
    ...
    def _pad_seg(self, results):
        ...
        ``results['pad_shape']``."""
        for key in results.get('seg_fields', []):
            results[key] = mmdet.cv_core.impad(
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.scale_factor != 1:
                results[key] = mmdet.cv_core.imrescale(
                    results[key],
                    self.scale_factor,
                    interpolation='nearest',
                    backend=self.backend)
...
class Albu(object):
    ...
    def albu_builder(self, cfg):
        ...
        if mmdet.cv_core.is_str(obj_type):
