...
from ..file.file_io import PathManager
...
class Visualizer:
    ...
    def __init__(self, img_rgb, metadata=None, scale=1.0, instance_mode=ColorMode.IMAGE):
        ...
        self.image2color = None
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("class", None):
            image2color = dict()
            for cat in self.metadata["class"]:
    ...
    def draw_instance_annotations(self, annotations, alpha=0.8):
        gt_bboxes = annotations.gt_bboxes
        gt_labels = annotations.gt_labels
        colors = None
        names = self.metadata.get("class")
        if self.image2color is not None:
            colors = [
                self.image2color[names[c]] for c in gt_labels
            ]
        ...
        self.overlay_instances(
            labels=labels, boxes=gt_bboxes, masks=None, keypoints=None, assigned_colors=colors, alpha=alpha,
        )
        return self.output
