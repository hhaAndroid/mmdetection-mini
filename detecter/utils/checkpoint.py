...
from cvcore.file import PathManager
from termcolor import colored
from torch.nn.parallel import DataParallel, DistributedDataParallel
from cvcore import Hook, HOOKS
...
class PeriodicCheckpointer(Hook):
    """
    ...
    def after_train_iter(self, runner):
        if not self.by_epoch:
            self.step(runner.iter)

    def after_train_epoch(self, runner):
        if self.by_epoch:
            self.step(runner.epoch)
    ...
    def step(self, iteration: int, **kwargs: Any) -> None:
        ...
        additional_state = {"iter": iteration}
...
class DetectionCheckpointer(Checkpointer):
    ...
    def __init__(self, model, save_dir="", *, save_to_disk=None, **checkpointables):
        is_main_process = dist_comm.is_main_process()
        super().__init__(
            model,
            save_dir,
    ...
    def load(self, path, *args, **kwargs):
        ...
        if path and isinstance(self.model, DistributedDataParallel):
            ...
            all_has_file = dist_comm.all_gather(has_file)
